...
def expected_stats(cls: str, race: str, level: int) -> dict[str, int]:
    base_str, base_dex, base_int = CLASS_BASE[cls]
    race_str, race_dex, race_int = RACE_BONUS[race]
    level_gain = level - 1
    level_one_str = base_str + race_str
    return {
        "level": level,
        "xp": 0,
        "xp_to_next": expected_xp_to_next(level),
        "strength": level_one_str + level_gain,
        "dexterity": base_dex + race_dex + level_gain,
        "intelligence": base_int + race_int + level_gain,
        "max_hp": 20 + level_one_str * 2 + 5 * level_gain,
    }
