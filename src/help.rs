...
const TOPICS: &[HelpTopic] = &[
    ...
    HelpTopic {
        ...
        usage: "sq status [--full]",
        details: "Prints your name, class, race, level and XP bar, current/max HP, gold, prestige tier, equipped weapon/armor/ring, and the zone derived from your current directory. By default the inventory list is omitted. Pass `--full` to append the same listing `sq inventory` would produce immediately below the sheet — useful when you want a single-screen snapshot before equipping or selling. The alias `sq stat` is identical to `sq status` and also accepts `--full`.",
        examples: &[
            "sq status",
            "sq stat",
            "sq status --full",
